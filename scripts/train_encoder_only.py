...
def train_trial(trial: dict[str, Any], cache: dict[str, Any], indices: dict[str, np.ndarray], device: torch.device, output_root: Path) -> dict[str, Any]:
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=int(config["epochs"])) if str(config["scheduler"]).lower() == "cosine" else None
    ...
    for epoch in range(1, int(config["epochs"]) + 1):
        ...
        if improved:
            ...
            torch.save({"model_state_dict": cloned_model_state(model), "config": config, "feature_mean": mean, "feature_std": std, "best_epoch": best_epoch, "best_val_macro_f1": best_score}, run_dir / "best.pt")
        else:
            waiting += 1
    ...
    torch.save({"model_state_dict": last_state, "config": config, "feature_mean": mean, "feature_std": std, "last_epoch": len(history), "best_epoch": best_epoch, "best_val_macro_f1": best_score}, run_dir / "last.pt")
    ...
    summary = {"run_id": run_id, "name": config["name"], "modalities": ",".join(config["modalities"]), "input_dim": config["input_dim"], "best_epoch": best_epoch, "best_val_macro_f1": best_score, **test_metrics, "seconds": time.time() - started, "run_dir": str(run_dir), "artifacts": ["config.json", "runtime.json", "data_context.json", "history.csv", "metrics.csv", "test_predictions.csv", "test_per_class_metrics.csv", "best.pt", "last.pt"]}
